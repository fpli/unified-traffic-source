DROP TABLE IF EXISTS p_soj_cl_t.temp_csess1a_v9;

-- Create a new table with the same schema as the original table
CREATE TABLE p_soj_cl_t.temp_csess1a_v9 USING delta AS
SELECT
  ev.guid,
  ev.session_skey,
  CAST(
    CASE
      WHEN ev.session_start_dt BETWEEN '2018-05-23' AND '2018-05-24'
		  AND CAST(sojlib.soj_nvl(ev.client_data, 'TPool') AS STRING) = 'r1rover'
		  AND sojlib.soj_nvl(ev.soj, 'es') IS NOT NULL
		  AND sojlib.is_integer(sojlib.soj_nvl(ev.soj, 'es')) = 1
		  AND CAST(sojlib.soj_nvl(ev.soj, 'es') AS SMALLINT) <> ev.site_id
		  AND app_id IN (2878, 1462, 2571)
  ...
  ev.pn,
  CASE
    WHEN ev.app_id IN (1462, 2878, 2571, 35023, 35024) THEN ev.app_id
    ELSE NULL
  END AS app_id,
  CASE
    ...
    WHEN
		lower(ev.sQr) LIKE ANY ('%.html','%.asp','%.jsp','%.gif','%.png','%.pdf','%.htm','%.php','%.cgi','%.jpeg','%.swf','%.txt','%.wav'
			,'%.zip','%.flv','%.dll','%.ico','%.jpg','null','undefined','%hideoutput%')
    ...
    WHEN ev.page_id IN (1702440,2043183,2043216,2051542,2051322,2051319,2052193,2051542,2052317,3693,2047675,2054574,2057587,2052197,2049334,2052122,2051865,4853,2504330,2504331,5410)
    		AND ev.web_server LIKE 'rover.ebay.%'
    ...
    WHEN ev.page_id IN (5713, 2053584, 6024, 2053898, 6053, 2054900)
    	AND (sojlib.soj_nvl(ev.soj, 'page') NOT IN ('ryprender','cyp','success','pots','error','rypexpress','MainCheckoutPage') OR sojlib.soj_nvl(ev.soj, 'page') IS NULL)
    ...
    WHEN ev.page_id IN (2053898)
        AND (
          sojlib.soj_nvl(ev.url_query_string, 'page') NOT IN ('MainCheckoutPage','CheckoutPaymentSuccess','CheckoutPayPalWeb','PaymentSent','CheckoutPaymentMethod','Autopay','CheckoutPayPalError1','CheckoutPaymentFailed')
          OR sojlib.soj_nvl(ev.soj, 'page') IS NULL)
    ...
    WHEN ev.page_id = 2376473 AND sojlib.soj_nvl(ev.soj, 'app') in ('1462', '2878') AND sojlib.soj_nvl(ev.soj, 'mav') IS NOT NULL
    	AND sojlib.soj_replace_char(sojlib.soj_nvl(ev.soj, 'mav'), '01234567890.', '') = ''
    	AND sojlib.soj_nvl(ev.soj, 'mav') LIKE ('5.%')
    	AND CAST(sojlib.soj_list_get_val_by_idx(sojlib.soj_nvl(ev.soj, 'mav'), '\\.', 2) AS INTEGER) >= 36
  ...
  CAST(
    CASE
      WHEN CHAR_LENGTH(ev.n) = 32 THEN ev.n
      ELSE NULL
    END AS STRING
  ) AS cguid,
  ev.siid AS siid,
  ...
  CASE
    ...
    WHEN sojlib.soj_get_url_path('http://x.ebay.com' || ev.url_query_string) LIKE '%/i/%'
		AND sojlib.is_decimal(sojlib.soj_list_last_element(sojlib.soj_get_url_path('http://x.ebay.com' || ev.url_query_string),'/'),18,0) = 1
			THEN CAST(sojlib.soj_list_last_element(sojlib.soj_get_url_path('http://x.ebay.com' || ev.url_query_string), '/' ) AS DECIMAL(18, 0))
    WHEN sojlib.soj_get_url_path('http://x.ebay.com' || ev.url_query_string) LIKE '%/itm/%'
		AND sojlib.is_decimal(sojlib.soj_list_last_element(sojlib.soj_get_url_path('http://x.ebay.com' || ev.url_query_string),'/'), 18, 0) = 1
			THEN CAST(sojlib.soj_list_last_element(sojlib.soj_get_url_path('http://x.ebay.com' || ev.url_query_string),'/') AS DECIMAL(18, 0))
    WHEN sojlib.soj_get_url_path('http://x.ebay.com' || ev.url_query_string) LIKE '%/p/%'
    	AND sojlib.is_decimal(sojlib.soj_nvl(sojlib.soj_get_url_params('http://x.ebay.com' || ev.url_query_string),'itm'),18,0) = 1
  ...
  CAST(
    CASE
      ...
      WHEN sojlib.soj_nvl(sojlib.soj_get_url_params('http://rover.ebay.com' || ev.url_query_string), 'nkw') IS NOT NULL
	  	THEN sojlib.soj_collapse_whitespace(sojlib.soj_replace_char(sojlib.soj_nvl(sojlib.soj_get_url_params('http://rover.ebay.com' || ev.url_query_string),'nkw'),'+',' '))
      ELSE NULL
    END AS STRING
  ) sqr2,
  CAST(
    CASE
      WHEN sojlib.soj_get_url_path('http://x.ebay.com' || ev.url_query_string) LIKE '/b/%'
      	AND sojlib.is_decimal(replace(sojlib.soj_list_last_element(sojlib.soj_get_url_path('http://x.ebay.com' || ev.url_query_string),'/'),'bn_',''),18,0) = 1
      ...
      CAST(
        CASE
          ...
          WHEN sojlib.soj_parse_clientinfo(ev2.client_data, 'Agent') LIKE 'eBayiPad/%'
		  		THEN 2878
          WHEN sojlib.soj_parse_clientinfo(ev2.client_data, 'Agent') LIKE 'ebayUserAgent/ebayMotorsIOS%'
		  		THEN 35023
      ...
      (
        SELECT
          ...
          CASE
            ...
            WHEN ev3.page_id IN (2523511,2523513,2546487,2523532,2523526,2523535,2508507,2368479,2239237,2255925,2056812,2368482,2500857,2523519,2546490,2523513)
            	 AND sojlib.is_decimal(sojlib.soj_nvl(ev3.soj, 'buyer_id'), 12, 0) = 1
          ...
          TRIM(
            CAST(
              COALESCE(
                sojlib.soj_nvl(ev3.soj, 'user_name'),
                sojlib.soj_nvl(ev3.soj, 'userid'),
                sojlib.soj_nvl(sojlib.soj_get_url_params('http://xxx.ebay.com' || ev3.url_query_string), 'un'),
                sojlib.soj_nvl(sojlib.soj_get_url_params('http://xxx.ebay.com' || ev3.url_query_string), 'userName')
              ) AS STRING
            )
          ) AS user_name,
          ...
          CAST(sojlib.soj_list_get_val_by_idx(replace(sojlib.soj_parse_clientinfo(ev3.client_data, 'ForwardedFor'),';',','),',',1) AS STRING) AS fwdfor1,
		  CAST(sojlib.soj_list_get_val_by_idx(replace( sojlib.soj_parse_clientinfo(ev3.client_data, 'ForwardedFor'), ';', ','), ',', 2) AS STRING) AS fwdfor2,
          CAST(sojlib.soj_list_get_val_by_idx(replace(sojlib.soj_parse_clientinfo(ev3.client_data, 'ForwardedFor'), ';', ','), ',', 3) AS STRING) AS fwdfor3,
          CAST(sojlib.soj_nvl(ev3.soj, 'pfn') AS STRING) AS pfn
        FROM
          ubi_v.ubi_event ev3
        WHERE
          ev3.session_start_dt = '2024-06-01'
          AND ev3.guid NOT IN (
            '',
            '13a2f8c31870acb0ae7c4cdafff04dc4',
            '25c9c9ef184a8c6d771c29e001911a5b',
            '27b98d8018608e009bd502c001915456',
            '5a3896c31870a9b124f76701fff7de0b',
            '81e7f94218a0a51285caed3dff9e052e',
            '8715d74917c0a9fc3c1ee016ffd0b80e',
            '95baed0d184ebb29d9d0609001f7f543',
            'bc7184a218ab26d7ba2c928001a1b26a',
            'c06b3c1d1800a7b21cef3cb8ffdba02f',
            'cf26dacd18f0a0ac45cbad41fffd1ae1',
            'd5958dd318f0a6ade076a02efffc1e27',
            'd720765b183a29a0effc9d8001e9b35b',
            'dc8853a117b49bcdfb890c800165b6ca'
          ) /* Remove skewed GUIDs from the query to cut down on long runtimes */
          AND ev3.page_id NOT IN (
            2608,
            2627,
            2774,
            2835,
          ...
          AND sojlib.soj_replace_char(ev3.guid, '01234567890abcdefABCDEF', '') = '' /* Remove bad GUIDs */
      ) ev2
  ) ev;