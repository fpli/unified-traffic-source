SELECT 'Now waiting for ubi_event_skew...';

DROP TABLE IF EXISTS p_soj_cl_t.temp_csess1a3_v9;
CREATE TABLE p_soj_cl_t.temp_csess1a3_v9 USING delta
SELECT
	s.guid,
	s.session_skey,
	s.session_start_dt
FROM
	ubi_v.ubi_session s
WHERE
	s.session_start_dt = '2024-06-01'
	AND s.guid NOT IN ('','13a2f8c31870acb0ae7c4cdafff04dc4','25c9c9ef184a8c6d771c29e001911a5b','27b98d8018608e009bd502c001915456','5a3896c31870a9b124f76701fff7de0b','81e7f94218a0a51285caed3dff9e052e','8715d74917c0a9fc3c1ee016ffd0b80e','95baed0d184ebb29d9d0609001f7f543','bc7184a218ab26d7ba2c928001a1b26a','c06b3c1d1800a7b21cef3cb8ffdba02f','cf26dacd18f0a0ac45cbad41fffd1ae1','d5958dd318f0a6ade076a02efffc1e27','d720765b183a29a0effc9d8001e9b35b','dc8853a117b49bcdfb890c800165b6ca')
	/* Remove skewed GUIDs from the query to cut down on long runtimes */
	AND s.bot_flag IN (12,15)
GROUP BY 1,2,3;

DROP TABLE IF EXISTS p_soj_cl_t.temp_csess1a4_v9;
CREATE TABLE p_soj_cl_t.temp_csess1a4_v9 USING delta
SELECT
	b.guid,
	b.session_skey,
	b.session_start_dt,
	SUM(
		CASE
			WHEN sojlib.soj_nvl(b.soj, 'bti') IS NOT NULL
				THEN 1
			ELSE 0
		END) AS bti_recs,
	SUM(
		CASE
			WHEN sojlib.soj_nvl(b.soj, 'ofid') IS NOT NULL
				THEN 1
			ELSE 0
		END) AS ofid_recs,
	SUM(
		CASE
			WHEN sojlib.soj_nvl(b.soj, 'watch') = '1'
				THEN 1
			ELSE 0
		END) AS wtch_recs,
	SUM(
		CASE
			WHEN sojlib.soj_nvl(b.soj, 'addtocart') = '1'
				THEN 1
			ELSE 0
		END) AS atc_recs
	FROM
		ubi_v.ubi_event_skew b
	WHERE
		b.session_start_dt = '2024-06-01'
		and b.page_id in (2047937,2355844,2356359,2490003,2490004,2493971,2494953,2510218,2510395,2510396,2510398,2510400,2510401,2510402,2510403,2523513,2544473,2544802,2545339,2545343,2545345,2561045,2561046,2566990,3221897,3221898,3727692,3733482,3733484,3733485,3733487,3827022)
		AND ( sojlib.soj_nvl(b.soj, 'bti') IS NOT NULL OR sojlib.soj_nvl(b.soj, 'ofid') IS NOT NULL OR sojlib.soj_nvl(b.soj, 'watch') = '1' OR sojlib.soj_nvl(b.soj, 'addtocart') = '1' )
	GROUP BY 1,2,3;

DELETE t1a4
FROM
	p_soj_cl_t.temp_csess1a4_v9 t1a4,
	p_soj_cl_t.temp_csess1a3_v9 t1a3
WHERE
	t1a4.guid = t1a3.guid
	and t1a4.session_skey = t1a3.session_skey
	and t1a4.session_start_dt = t1a3.session_start_dt;

INSERT INTO p_soj_cl_t.temp_csess1a3_v9 SELECT guid, session_skey, session_start_dt FROM p_soj_cl_t.temp_csess1a4_v9;
...
WHERE
	t1a.guid = t1a3.guid
	and t1a.session_skey = t1a3.session_skey
	and t1a.session_start_dt = t1a3.session_start_dt;

DROP TABLE IF EXISTS p_soj_cl_t.temp_csess1bot_v9;
CREATE TABLE p_soj_cl_t.temp_csess1bot_v9 USING PARQUET
SELECT
	ev3.*
FROM
	ubi_v.ubi_event_skew ev3
	INNER JOIN p_soj_cl_t.temp_csess1a3_v9 s
...
DROP TABLE IF EXISTS p_soj_cl_t.temp_csess1a5_v9;
CREATE TABLE p_soj_cl_t.temp_csess1a5_v9 USING PARQUET
SELECT
	ev.guid,
	ev.session_skey,
	CAST(
	...
	CASE
		...
		ELSE 1
	END /* Exclude invalid pages */ AS partial_valid_page,
	CASE
			...
			FROM (
				SELECT
					...
					CASE
						...
						WHEN ev3.url_query_string LIKE '%/%'
							THEN '/' || ev3.url_query_string ELSE '/?' || COALESCE(ev3.url_query_string, '')
					...
					CASE
						...
						WHEN ev3.page_id IN (2523511,2523513,2546487,2523532,2523526,2523535,2508507,2368479,2239237,2255925,2056812,2368482,2500857,2523519,2546490,2523513) AND sojlib.is_decimal(sojlib.soj_nvl(ev3.soj, 'buyer_id'), 12, 0) = 1
				...
				FROM
					p_soj_cl_t.temp_csess1bot_v9 ev3
				WHERE
